# Na versão <= 1.8 só é aceito hashs nesse formato
# {
#   :nome => 'Leandro',
#   :idade => 21
# }

# Hash
clientes = {
  1 => {
    nome: 'Lucas',
    data_de_cadastro: '25/11/2018',
    cidade: 'Roseira-SP'
  },
  2 => {
    nome: 'Pedro',
    data_de_cadastro: '15/11/2016',
    cidade: 'São Paulo-SP'
  },
  3 => {
    nome: 'Fábio',
    data_de_cadastro: '15/11/2018',
    cidade: 'Rio de Janeiro-RJ'
  }
}

# captura os parametros passado pelo comando abaixo:
# Ex: ruby registro_de_clientes 1
id_do_cliente  = ARGV.first.to_i

puts "Buscando informações do cliente ##{id_do_cliente}..."

# segundos
sleep 3

# recuperando do hash clientes a chave correspondente ao valor
cliente = clientes[id_do_cliente]

# variável cliente diferente de nulo
if cliente != nil
  puts "Nome: #{cliente[:nome]}"
  puts "Data de cadastro: #{cliente[:data_de_cadastro]}"
  puts "Cidade: #{cliente[:cidade]}"
  puts
  puts "Programa finalizado."
else
  puts "Cliente não encontrado."
end